...
public class IFTAReports extends ConfigurationSetup {
	
	public static Logger log = Logger.getLogger(IFTAReports.class);
	...
	@Test(priority=1, groups = "IFTA Report")
	public void GETIFTAReport(ITestContext context) throws IOException 
	{
			
		String targetAPI = APIurl+"client-services/api/v1/fms/ifta-report/";
		
		log.info("Consuming API : "+targetAPI);	
		
		
		AddHeaders("IFTA TRUCKX-FMS-ID");
		...
		AddQueryParameter("IFTA start_date");

		AddQueryParameter("IFTA end_date");
		
		AddQueryParameter("IFTA truck_uuid");
		
		AddQueryParameter("IFTA userId");
		...
		try
		{
			MatcherAssert.assertThat(res.body().asPrettyString(), JsonSchemaValidator.matchesJsonSchema(IFTAReportSchema));
