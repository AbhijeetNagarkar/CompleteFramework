...
public class WebPageTripHistory {

	WebDriver driver;
	
	WebDriverWait wait;

	public static Logger log = Logger.getLogger(WebPageTripHistory.class);
	
	public WebPageTripHistory(WebDriver driverinstance)
	{
	...
	public boolean SearchandVerifyRecords() throws InterruptedException {
		...
		List<WebElement> ele = driver.findElements(By.xpath("//tr[@class=\"rc-table-row rc-table-row-level-0 text-base text-gray-400 bg-white border-b border-gray-100 h-12 w-full hover:bg-blue-tablerow focus:hover:bg-blue-tablerow rounded-md`\"]//a"));
		
		for(WebElement element : ele)
		{
			if(element.getText().toLowerCase().contains("krishna"))
			{
				continue;
			}
			else
